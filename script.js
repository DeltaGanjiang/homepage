...
Object.assign(english,{orbTitle:'Gan Jiang',orbSubtitle:'A science agent for powder X-ray diffraction',orbCta:'Enter the site <span aria-hidden="true">↓</span>'});
...
if (xrdCanvas) {
  ...
  function draw(ms){const w=field.clientWidth,h=field.clientHeight,t=ms/1000,scene=scenes[active],left=42,chartW=w*.42,cy=h*.48,structureX=w*.76,s=Math.min(w,h)*.2;ctx.clearRect(0,0,w,h);ctx.strokeStyle="rgba(142,180,255,.1)";ctx.lineWidth=1;for(let x=0;x<w;x+=34){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke();}for(let y=0;y<h;y+=34){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke();}ctx.fillStyle="#77f6d2";ctx.font="500 10px DM Mono";ctx.fillText("MEASURED PATTERN / 2θ",left,34);ctx.fillStyle="#eaf1ff";ctx.font="600 23px Noto Serif SC";const labels=document.documentElement.lang==="en"?[["Layered oxides","Phase identification"],["Perovskites","Structure refinement"],["Multiphase systems","Quantitative decomposition"]][active]:[scene.name,scene.task];ctx.fillText(labels[0],left,64);ctx.fillStyle="#8fa4cc";ctx.font="500 10px DM Mono";ctx.fillText(labels[1].toUpperCase(),left,84);ctx.strokeStyle="rgba(132,177,255,.35)";ctx.strokeRect(left,105,chartW,h-162);for(let n=1;n<5;n++){ctx.globalAlpha=.55;ctx.beginPath();ctx.moveTo(left,105+n*(h-162)/5);ctx.lineTo(left+chartW,105+n*(h-162)/5);ctx.stroke();ctx.globalAlpha=1;}peaks(left+16,h*.75,chartW-32,h*.43,scene,t,true);ctx.fillStyle="#77f6d2";ctx.font="500 10px DM Mono";ctx.fillText("GAN JIANG / ANALYSIS PATH",w*.52,34);for(let n=0;n<3;n++){const y=cy+(n-1)*36,progress=(t*.18+n*.33)%1;ctx.strokeStyle=`rgba(119,246,210,${.16+n*.1})`;ctx.beginPath();ctx.moveTo(left+chartW+22,h*.62);ctx.bezierCurveTo(w*.56,h*.62-y*.05,w*.6,y,w*.68,y);ctx.stroke();dot(w*.49+progress*w*.2,cy+(n-1)*18,3.5,"#bdfcff",.85);}ctx.fillStyle="rgba(119,246,210,.1)";ctx.beginPath();ctx.arc(structureX,cy,s*1.45,0,Math.PI*2);ctx.fill();if(active===0)layered(structureX,cy,s,scene,t,true);if(active===1)perovskite(structureX,cy,s,scene,t,true);if(active===2)mixture(structureX,cy,s,scene,t,true);ctx.fillStyle="#77f6d2";ctx.font="500 10px DM Mono";ctx.fillText("STRUCTURE HYPOTHESIS",structureX-s, h-38);ctx.fillStyle="#8fa4cc";ctx.fillText(scene.note,structureX-s,h-20);requestAnimationFrame(draw);}
...
Object.assign(english,{analysisEyebrow:"One measurement, one chain of evidence",analysisTitle:"Gan Jiang connects characterization to structure.",analysisText:"From raw diffraction peaks to candidate phases, structural models, and reviewable validation, the analysis advances along one connected path.",analysisStepOne:"Read the pattern",analysisStepOneDetail:"peaks · intensity · background",analysisStepTwo:"Gan Jiang reasons",analysisStepTwoDetail:"match · decompose · validate",analysisStepThree:"Deliver a conclusion",analysisStepThreeDetail:"structure candidate · evidence"});
